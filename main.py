...
from onion.l2.g_filemanager_pickle import g_files_arr, s_data_dump, g_data_load, g_data_loads
...
async def main():
    for symbol in\
        ...
        # await g_files_arr():
        # ["BTCUSDT"]:
        if symbol not in await g_files_arr():
            print(symbol)
            
            # data = (await g_data_load(name=f"data_pack/data_backtest/{symbol}")).copy()
            ...
            if data is not None:
                ...
                print("get x")
                data["train"] = await g_y_train_l1(data)
                data["train"] = await g_y_train_l2(data)
                print("get train")
